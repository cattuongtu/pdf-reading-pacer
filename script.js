...
var textItems;
var separateWords;
var _SAVE_START_END = document.querySelector("#save-start-end");
_SAVE_START_END.disabled = true;

// INITIALIZE AND LOAD PDF
async function showPDF(pdf_url) {
  document.querySelector("#pdf-loader").style.dislpay = "block";
  ...
  try {
    ...
    document.querySelector("#page-count-container").style.display =
      "inline-block";
...
async function showPage(page_no) {
  ...
  document.querySelector("#text-layer").style.display = "none";
  ...
  var scale_required = { scale: _CANVAS.width / pdf_original_width };
...
document
  ...
  .addEventListener("click", function() {
    document.querySelector("#pdf-loader").innerHTML = "Loading pdf...";
    try {
      showPDF(
        `https://cors-anywhere.herokuapp.com/${
          document.querySelector("#get-pdf-link-entry").value
        }`
      );
      document.querySelector("#step-1").style.color = "#004df9";
    } catch (error) {
      alert(error.message);
    }
  });
...
// ORIGINAL PDF BUTTON
document.querySelector("#original-pdf").addEventListener("click", () => {
  _CANVAS.style.opacity = "1.0";
  _TEXT_LAYER.style.opacity = "0.0";
  _PACER_MODE = false;
});
...
var _START_PAGE_INPUT = document.querySelector("#start-page");
_START_PAGE_INPUT.defaultValue = 1;
var _END_PAGE_INPUT = document.querySelector("#end-page");
var _END_PAGE_CHECKBOX = document.querySelector("#end-page-checkbox");
...
// PACER MODE BUTTON
document.querySelector("#pacer-mode").addEventListener("click", () => {
  _CANVAS.style.opacity = "0.0";
  _TEXT_LAYER.style.opacity = "1.0";
  _PACER_MODE = true;
  document.querySelector("#step-2").style.color = "#004df9";
});

// CYAN HIGHLIGHT
document.querySelector("#cyan-color").addEventListener("click", () => {
  _HIGHLIGHT_COLOR = "cyan";
  document.querySelector("#step-5").style.color = "#004df9";
});

// YELLOW HIGHLIGHT
document.querySelector("#yellow-color").addEventListener("click", () => {
  _HIGHLIGHT_COLOR = "yellow";
  document.querySelector("#step-5").style.color = "#004df9";
});
...
document.querySelector("#start-pacing").addEventListener("click", () => {
  paceMultiplePages();
});

function paceMultiplePages() {
  document.querySelector("#step-5").style.color = "#004df9";
  document.querySelector("#step-6").style.color = "#004df9";

  while (_CURRENT_PAGE != _END_PAGE) {
